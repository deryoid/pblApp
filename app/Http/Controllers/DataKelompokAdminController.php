...
class DataKelompokAdminController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = Kelompok::with('periode')->withCount('mahasiswas')->latest();
    ...
    public function create(Request $request)
    {
        ...
        // ⬇️ Ambil id mahasiswa yang sudah terpakai di periode terpilih
        $takenMahasiswaIds = DB::table('kelompok_mahasiswa')
            ->where('periode_id', $selectedPeriodeId)
            ->pluck('mahasiswa_id');

        // ⬇️ Tampilkan hanya mahasiswa yang BELUM tergabung di periode tsb
    ...
    public function store(Request $request)
    {
        ...
        // Generate nama + simpan
        $namaKelompok = null; $attempt = 0; $maxAttempt = 25;
        do {
            $attempt++;
            ...
            try {
                DB::transaction(function() use ($periodeId,$candidate,$mahasiswa,$rows,$validated,&$namaKelompok) {
    ...
    public function edit(Request $request, string $uuid)
    {
        ...
        // ⬇️ Mahasiswa yang sudah terpakai di periode ini, KECUALI yang ada di kelompok ini
        $takenExceptThis = DB::table('kelompok_mahasiswa')
            ->where('periode_id', $selectedPeriodeId)
            ->where('kelompok_id', '<>', $kelompok->id)
            ->pluck('mahasiswa_id');

        // ⬇️ Tampilkan semua yang belum terpakai + anggota kelompok ini sendiri
