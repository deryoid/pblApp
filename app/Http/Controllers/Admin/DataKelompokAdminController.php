...
class DataKelompokAdminController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = Kelompok::with('periode')
            ...
            ->with(['evaluasiMasters' => function ($q) use ($qPeriode) {
                if ($qPeriode) {
                    $q->where('periode_id', $qPeriode);
                }
            }]);
        if ($qPeriode) {
            $query->where('periode_id', $qPeriode);
        }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($mahasiswa as $m) {
            if ($m->kelompoks()->wherePivot('periode_id', $periodeId)->exists()) {
                $sudah[] = $m->nim;
            }
        }
        if ($sudah) {
        ...
        if (! $ketuaNim && $rows->count()) {
            ...
            if (isset($attach[$firstId])) {
                $attach[$firstId]['role'] = 'Ketua';
            }
        }
            ...
            DB::transaction(function () use ($validated, $periodeId, $attach, &$namaKelompok) {
                ...
                do {
                    ...
                    try {
                        ...
                        if (! $isUnique || $attempt >= $maxAttempt) {
                            throw $e;
                        }
                        // retry next number
                    }
