...
class PenilaianEvaluasiController extends Controller
{
    ...
    public function rekap(int $sesiId)
    {
        ...
        // untuk header & urutan kolom
        // fallback: jika kolom 'position' belum ada, urutkan berdasarkan 'code' / 'id'
        $groupsQuery = \App\Models\EvaluationRubricGroup::query();
        if (\Illuminate\Support\Facades\Schema::hasColumn('evaluation_rubric_groups', 'position')) {
            $groupsQuery->orderBy('position');
        } else {
            $groupsQuery->orderBy('code');
        }
        $groups = $groupsQuery->get()->keyBy('code');

        $indsQuery = \App\Models\EvaluationRubricIndicator::orderBy('group_code');
        if (\Illuminate\Support\Facades\Schema::hasColumn('evaluation_rubric_indicators', 'position')) {
            $indsQuery->orderBy('position');
        } else {
            $indsQuery->orderBy('id');
        }
        $indsByGroup = $indsQuery->get()->groupBy('group_code');
